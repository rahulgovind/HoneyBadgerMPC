...
tmux new-session     "${CMD} -d -f ${CONFIG_PATH}.0.json; sh" \; \
     splitw -h -p 66 "${CMD} -d -f ${CONFIG_PATH}.1.json; sh" \; \
     splitw -h -p 50 "${CMD} -d -f ${CONFIG_PATH}.2.json; sh" \; \
     selectp -t 0 \; \
     splitw -v -p 66 "${CMD} -d -f ${CONFIG_PATH}.3.json; sh" \; \
     splitw -v -p 50 "${CMD} -d -f ${CONFIG_PATH}.6.json; sh" \; \
     selectp -t 3 \; \
     splitw -v -p 66 "${CMD} -d -f ${CONFIG_PATH}.4.json; sh" \; \
     splitw -v -p 50 "${CMD} -d -f ${CONFIG_PATH}.7.json; sh" \; \
     selectp -t 6 \; \
     splitw -v -p 66 "${CMD} -d -f ${CONFIG_PATH}.5.json; sh" \; \
     splitw -v -p 50 "${CMD} -d -f ${CONFIG_PATH}.8.json; sh" \; \